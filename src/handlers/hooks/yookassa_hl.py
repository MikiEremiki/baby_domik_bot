...
async def processing_ticket_paid(update, context: 'ContextTypes.DEFAULT_TYPE'):
    ...
    try:
        if is_website and int_chat_id != 0:
            user_data = context.application.user_data[int_chat_id]
        ...
        if is_admin_booking:
            ...
            try:
                user = user_data.get('user') if int_chat_id != 0 and user_data else None
        ...
        if (
            int_chat_id != 0
            and getattr(context.application, 'persistence', None)
            and int_chat_id in context.application.user_data
        ):
            try:
                context.application.mark_data_for_update_persistence(user_ids=[int_chat_id])
            except Exception as e:
                webhook_hl_logger.error(
                    f'Ошибка при пометке данных для persistence: {e}')
